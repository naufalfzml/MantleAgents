...
function TimelineRow({ event, onViewLogs }: { event: any, onViewLogs: () => void }) {
  ...
  if (event.eventType === 'trade') {
    ...
    const amount = event.amountUsd ? `$${event.amountUsd.toFixed(2)}` : null;
    if (amount) {
       impact = <span className="font-mono font-medium text-orange-400 text-sm">-{amount}</span>;
    }

    statusBadge = (
        <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-orange-500/10 text-orange-500 border border-orange-500/20 leading-none">
            Success
        </span>
    );
